...
class Document:
    ...
    def from_json(cls, js):
        ...
        sentences = update_sentences_with_event_clusters(
            sentences, event_clusters)
    ...
    def split(self, max_tokens_per_doc):
        ...
        if self.clusters is not None or self.predicted_clusters is not None:
            raise NotImplementedError(
                "Splitting documents with coreference annotations not implemented.")
        if self.event_clusters is not None or self.predicted_event_clusters is not None:
            raise NotImplementedError(
                "Splitting documents with event coreference annotations not implemented.")
        ...
        doc_keys = [f"{self.doc_key}_SPLIT_{i}" for i in range(
            len(sentence_groups))]
...
class Sentence:
    def __init__(self, entry, sentence_start, sentence_ix):
        ...
        if "ner" in entry:
            ...
            self.ner_dict = {
                entry.span.span_sent: entry.label for entry in self.ner}
        else:
            self.ner = None
            self.ner_dict = None
        ...
        if "predicted_events" in entry:
            self.predicted_events = PredictedEvents(
                entry["predicted_events"], self)
        else:
    ...
    def to_json(self):
        ...
        if self.predicted_ner is not None:
            res["predicted_ner"] = [entry.to_json()
                                    for entry in self.predicted_ner]
        if self.relations is not None:
            res["relations"] = [entry.to_json() for entry in self.relations]
        if self.predicted_relations is not None:
            res["predicted_relations"] = [entry.to_json()
                                          for entry in self.predicted_relations]
...
class EventBase(ABC):
    def __init__(self, event, sentence, sentence_offsets=False):
        trig = event[0]
        args = event[1:]
        self.trigger = self.trigger_constructor(
            trig, sentence, sentence_offsets)

        self.arguments = []
        for arg in args:
            this_arg = self.argument_constructor(
                arg, self.trigger.label, sentence, sentence_offsets)
    ...
    def __init__(self, events_json, sentence, sentence_offsets=False):
        ...
        for event in self.event_list:
            ...
            for argument in event.arguments:
                # (trigger_ix, (arg_start, arg_end))
                arg_key = (trigger_key, argument.span.span_sent)
...
class Cluster:
    def __init__(self, cluster, cluster_id, sentences, sentence_starts):
        ...
        for entry in cluster:
            try:
                sentence_ix = get_sentence_of_span(
                    entry, sentence_starts, n_tokens)
        ...
        if members_crossing_sentences:
            print(
                "Found a coreference cluster member that crosses sentence boundaries; skipping.")
